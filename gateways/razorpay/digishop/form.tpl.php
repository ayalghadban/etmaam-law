<?php
    ...
    * @copyright 2023
    * @version 6.20: form.tpl.php, v1.00 6/27/2023 1:26 PM Gewa Exp $
    *
    */
   if (!defined('_WOJO')) {
   ...
   $this->db->update(Digishop::qTable, array('order_id' => $razorpayOrderId))->where('user_sid', $this->auth->sesid, '=')->run();
?>
<form name="razorpayform" action="<?php echo Url::url('/validate');?>" method="POST">
   <input type="hidden" name="razorpay_payment_id" id="razorpay_payment_id">
   <input type="hidden" name="razorpay_signature"  id="razorpay_signature" >
   <!-- Any extra fields to be submitted with the form but not sent to Razorpay -->
   <input type="hidden" name="type" value="DIGI">
